...
function enviarMovimiento(id) {
  fetch("http://44.217.103.65/api/movimientos/", {
    method: "POST",
...
function detenerMovimiento() { enviarMovimiento(3); }

function enviarVelocidad(idVel) {
  fetch("http://44.217.103.65/api/velocidad/actual", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const ws = new WebSocket("ws://44.217.103.65:443");
...
ws.onmessage = (msg) => {
  ...
  if (data.event === "nuevo_movimiento") {
    document.getElementById("monitorMovimiento").innerHTML = `
      <div><span class="tag">Movimiento:</span> ${movimientosTexto[data.id_movimiento]}</div>
